...
function generateChatId() {
    do {
        partOne = Math.floor(Math.random() * (9999999999)).toString().padStart(9, "0");     //generate id part1 and pad
        partTwo = Math.floor(Math.random() * (99999999999)).toString().padStart(10, "0");   //generate id part2 and pad
        id = partOne + partTwo;                                                             //concatenate parts
        //id = "1";
        
        con.query("SELECT id FROM messages WHERE id = " + id, (err, result)=>{              //check if ID already exists
            if(err !== null) { console.log(err) };
    ...
    } while (idExists);                                                                     //if ID exists then try again
